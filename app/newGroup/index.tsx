...
const index = () => {
  ...
  const renderMember = ({ item }: { item: MemberProps }) => {
    const firstLetter = item.name.first.charAt(0).toUpperCase();
      ...
      >
        <View style={styles(colors).memberInfo}>
          {item.avatar ? (
            ...
            <View style={styles(colors).avatarFallback}>
              <TextR style={styles(colors).avatarLetter}>{firstLetter}</TextR>
            </View>
          )}
          ...
          !showSearch && (
            ...
            >
              <StackUI
                ...
                base={<Ionicons name="search" size={24} color={colors.primary } />}
              />
            </TouchableOpacity>
          )
        }
